...
const server = Bun.serve({
  ...
  routes: {
    ...
    [`/${API_PATROL_PREFIX}/*`]: async (request) => {
      ...
      const result = await use_case_factory.handle_request_use_case().execute(incoming_request) 
      return Response.json(result.body, { status: result.status_code, headers: result.headers })
    }
  }
})
